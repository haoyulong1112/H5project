<template>
  <div class="container">
    ...
    <div class="userInfo clearfloat">
      <div class="user">收货人：
        <span>{{expressList.consigner}}</span>
      </div>
      <div class="cellPhone">{{expressList.mobile}}</div>
    ...
    <div class="expInfo">
      <div class="expleft">
        <div class="express clearfloat">物流公司：
          <span>{{expressList.express}}</span>
        </div>
        <div class="express_no clearfloat">
            <div>快递单号：<span class="expressNo">{{expressList.expressNo}}</span></div>
        </div>
      </div>
      <div >
         <clipboard :text="expressList.expressNo" class="clipboard"></clipboard>
      </div>
    </div>
...
export default {
    ...
    created () {
        let express = window.expressList
        let taskId = this.$route.params.id

        for (let i = 0; i < express.length; i++) {
            if (taskId == express[i].userTaskId) {
                this.expressList = express[i].logistics
                this.expressInfo = JSON.parse(this.expressList.info)
            }
        }
    },
    methods: {
